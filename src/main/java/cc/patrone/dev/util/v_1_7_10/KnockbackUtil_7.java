package cc.patrone.dev.util.v_1_7_10;
...
public class KnockbackUtil_7 implements KnockbackUtil {
    @Override
    public void damageEntity(Player player, Entity target) {
        EntityPlayer ep = ((CraftPlayer) player).getHandle();
        net.minecraft.server.v1_7_R4.Entity nmsEntity = ((CraftEntity) target).getHandle();
        boolean disablePlayerCrits = false;
        float f = (float) ep.getAttributeInstance(GenericAttributes.e).getValue();
        byte b0 = 0;
        float f1;

        ItemStack itemInHand = ep.getEquipment(0);
        if (nmsEntity instanceof EntityLiving) {
            f1 = EnchantmentManager.a(itemInHand, ((EntityLiving) nmsEntity).getMonsterType());
        } else {
            f1 = EnchantmentManager.a(itemInHand, EnumMonsterType.UNDEFINED);
        }

        int i = b0 + EnchantmentManager.getEnchantmentLevel(Enchantment.KNOCKBACK.id, itemInHand);
        ...
        if (f > 0.0F || f1 > 0.0F) {
            boolean flag = !disablePlayerCrits && ep.fallDistance > 0.0F && !ep.onGround && !ep.h_() && !ep.M() && !ep.hasEffect(MobEffectList.BLINDNESS) && ep.vehicle == null && nmsEntity instanceof EntityLiving; // PaperSpigot
